...
// GetAccessKey 获取B站Access Key（从环境变量读取）
func GetAccessKey() string {
	key := config.GetBiliAccessKey()
	if key == "" {
		log.Println("警告: 未找到BILI_ACCESS_KEY环境变量")
		return ""
	}
	return key
}

// GetAccessKeySecret 获取B站Access Key Secret（从环境变量读取）
func GetAccessKeySecret() string {
	secret := config.GetBiliSecretKey()
	if secret == "" {
		log.Println("警告: 未找到BILI_ACCESS_KEY_SECRET环境变量")
		return ""
	}
	return secret
}
...
// ApiRequest http request demo
// 这是一个通用的HTTP请求方法，用于向B站开放平台发送POST请求
